/* Process model C form file: WSN_MAC_UP.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char WSN_MAC_UP_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 624EA60E 624EA60E 1 DESKTOP-RD4S7T2 51133 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1bcc 1                                                                                                                                                                                                                                                                                                                                                                                                    ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	WSN_MAC_UP_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((WSN_MAC_UP_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
static void update_alive_time(int source)
	...
	if(next == neighbor_table.node_address)
		{
		neighbor_table.active_time = ACTIVE_TIME;
		}
	FOUT;
...
static void csma_schedule_backoff ()
	...
	printf("Node %d  g_mac_backoff_exponent = %d backoff_periods = %f\n",g_mac_node_id,g_mac_backoff_exponent,backoff_periods);
...
extern "C" {
#endif
	void WSN_MAC_UP (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_WSN_MAC_UP_init (int * init_block_ptr);
	void _op_WSN_MAC_UP_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_WSN_MAC_UP_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_WSN_MAC_UP_alloc (VosT_Obtype, int);
	void _op_WSN_MAC_UP_svar (void *, const char *, void **);
...
void
WSN_MAC_UP (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	...
	FIN_MT (WSN_MAC_UP ());
		...
		FSM_ENTER ("WSN_MAC_UP")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "WSN_MAC_UP [init enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [init enter execs]", state0_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (0, "init", "WSN_MAC_UP [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "init", "waiting", "tr_3", "WSN_MAC_UP [init -> waiting : default / ]")
				/*---------------------------------------------------------*/



			/** state (scan) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "scan", state1_enter_exec, "WSN_MAC_UP [scan enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [scan enter execs]", state1_enter_exec)
				{
				int framecontrol;
				int type;
				double freq;
			...
			FSM_EXIT (3,"WSN_MAC_UP")


			/** state (scan) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "scan", "WSN_MAC_UP [scan exit execs]")


			/** state (scan) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [scan trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 8, state8_enter_exec, ;, "g_mac_scan_flag == NO_SCAN", "", "scan", "waiting", "tr_10", "WSN_MAC_UP [scan -> waiting : g_mac_scan_flag == NO_SCAN / ]")
				FSM_CASE_TRANSIT (1, 1, state1_enter_exec, ;, "default", "", "scan", "scan", "tr_14", "WSN_MAC_UP [scan -> scan : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (2, "associating", state2_enter_exec, "WSN_MAC_UP [associating enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [associating enter execs]", state2_enter_exec)
				{
				int type;
				int framecontrol;
				int dest;
				...
				if(BACKOFF_TIMER_EXPIRED)					//退避时间到
					...
					if(g_mac_pk_type!=NO_FRAME)
						...
						if(CH_FREE)
							...
							if(g_mac_current_time+g_mac_sending_time-g_mac_slot_start_time<0.3)
								{
								csma_schedule_backoff ();
								}
							}
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"WSN_MAC_UP")


			/** state (associating) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "associating", "WSN_MAC_UP [associating exit execs]")


			/** state (associating) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [associating trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 3, state3_enter_exec, ;, "C_SELF_ASSOCIATE_SUCCESS", "", "associating", "online", "tr_8", "WSN_MAC_UP [associating -> online : C_SELF_ASSOCIATE_SUCCESS / ]")
				FSM_CASE_TRANSIT (1, 8, state8_enter_exec, ;, "C_SELF_ASSOCIATE_FAIT", "", "associating", "waiting", "tr_18", "WSN_MAC_UP [associating -> waiting : C_SELF_ASSOCIATE_FAIT / ]")
				FSM_CASE_TRANSIT (2, 2, state2_enter_exec, ;, "default", "", "associating", "associating", "tr_21", "WSN_MAC_UP [associating -> associating : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (3, "online", state3_enter_exec, "WSN_MAC_UP [online enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [online enter execs]", state3_enter_exec)
				{
				Packet* pk2;
				Packet* pk;
				int type;
				
				//if(g_mac_node_id == 1) printf("Node %d  in 6\n",g_mac_node_id);
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (7,"WSN_MAC_UP")


			/** state (online) exit executives **/
			FSM_STATE_EXIT_UNFORCED (3, "online", "WSN_MAC_UP [online exit execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [online exit execs]", state3_exit_exec)
				{
				//if(g_mac_node_id == 1) printf("Node %d  out 6\n",g_mac_node_id);
			...
			FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [online trans conditions]", state3_trans_conds)
			FSM_INIT_COND (C_SELF_NEW_SLOT)
			FSM_TEST_COND (BACKOFF_TIMER_EXPIRED&&CSMA_MODE==1)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 7, state7_enter_exec, set_channel_start_receive ();, "C_SELF_NEW_SLOT", "set_channel_start_receive ()", "online", "slot_intrpt", "tr_24", "WSN_MAC_UP [online -> slot_intrpt : C_SELF_NEW_SLOT / set_channel_start_receive ()]")
				FSM_CASE_TRANSIT (1, 6, state6_enter_exec, ;, "BACKOFF_TIMER_EXPIRED&&CSMA_MODE==1", "", "online", "cca", "tr_26", "WSN_MAC_UP [online -> cca : BACKOFF_TIMER_EXPIRED&&CSMA_MODE==1 / ]")
				FSM_CASE_TRANSIT (2, 5, state5_enter_exec, ;, "C_STRM_MSG_FROM_FATHER_NODE", "", "online", "msg from up", "tr_29", "WSN_MAC_UP [online -> msg from up : C_STRM_MSG_FROM_FATHER_NODE / ]")
				FSM_CASE_TRANSIT (3, 4, state4_enter_exec, ;, "C_STRM_NWK_DATA", "", "online", "App_Msg_Up", "tr_31", "WSN_MAC_UP [online -> App_Msg_Up : C_STRM_NWK_DATA / ]")
				FSM_CASE_TRANSIT (4, 9, state9_enter_exec, ;, "C_STRM_NWK_MLME&&g_mac_node_status == ONLINE", "", "online", "MLME", "tr_35", "WSN_MAC_UP [online -> MLME : C_STRM_NWK_MLME&&g_mac_node_status == ONLINE / ]")
				FSM_CASE_TRANSIT (5, 12, state12_enter_exec, set_channel_start_send ();, "C_SELF_UP_TIME&&CSMA_MODE==0", "set_channel_start_send ()", "online", "send", "tr_56", "WSN_MAC_UP [online -> send : C_SELF_UP_TIME&&CSMA_MODE==0 / set_channel_start_send ()]")
				FSM_CASE_TRANSIT (6, 8, state8_enter_exec, ;, "C_SLEF_LEAVE", "", "online", "waiting", "tr_74", "WSN_MAC_UP [online -> waiting : C_SLEF_LEAVE / ]")
				FSM_CASE_TRANSIT (7, 3, state3_enter_exec, ;, "default", "", "online", "online", "tr_64", "WSN_MAC_UP [online -> online : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (4, "App_Msg_Up", state4_enter_exec, "WSN_MAC_UP [App_Msg_Up enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [App_Msg_Up enter execs]", state4_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (4, "App_Msg_Up", "WSN_MAC_UP [App_Msg_Up exit execs]")


			/** state (App_Msg_Up) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [App_Msg_Up trans conditions]", state4_trans_conds)
			FSM_INIT_COND (g_mac_sending_flag == 2)
			FSM_TEST_COND (g_mac_sending_flag != 2)
			FSM_TEST_LOGIC ("App_Msg_Up")
			FSM_PROFILE_SECTION_OUT (state4_trans_conds)

			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 3, state3_enter_exec, ;, "g_mac_sending_flag == 2", "", "App_Msg_Up", "online", "tr_30", "WSN_MAC_UP [App_Msg_Up -> online : g_mac_sending_flag == 2 / ]")
				FSM_CASE_TRANSIT (1, 12, state12_enter_exec, ;, "g_mac_sending_flag != 2", "", "App_Msg_Up", "send", "tr_69", "WSN_MAC_UP [App_Msg_Up -> send : g_mac_sending_flag != 2 / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (5, "msg from up", state5_enter_exec, "WSN_MAC_UP [msg from up enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [msg from up enter execs]", state5_enter_exec)
				{
				int framecontrol;
				int type;
				Packet* pk;
			...
			FSM_STATE_EXIT_FORCED (5, "msg from up", "WSN_MAC_UP [msg from up exit execs]")


			/** state (msg from up) transition processing **/
			FSM_TRANSIT_FORCE (3, state3_enter_exec, ;, "default", "", "msg from up", "online", "tr_70", "WSN_MAC_UP [msg from up -> online : default / ]")
				/*---------------------------------------------------------*/



			/** state (cca) enter executives **/
			FSM_STATE_ENTER_FORCED (6, "cca", state6_enter_exec, "WSN_MAC_UP [cca enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [cca enter execs]", state6_enter_exec)
				{
				/*
				int pkptr_size;
				if(C_SELF_UP_TIME)
				...
				if(C_SELF_SEND_COMPLETED)
					...
					if(g_mac_pk_type == DATA_FRAME||g_mac_pk_type == ACK_FRAME||g_mac_pk_type == MAC_CONTROL_FRAME||g_mac_pk_type == RETRANS_FRAME)
						...
						if(g_mac_current_time+g_mac_sending_time-g_mac_slot_start_time < 0.495)
							{
							send_msg();
							}
						else
			...
			FSM_STATE_EXIT_FORCED (6, "cca", "WSN_MAC_UP [cca exit execs]")


			/** state (cca) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [cca trans conditions]", state6_trans_conds)
			FSM_INIT_COND (CH_FREE&&!BACKOFF_LIMIT_REACHED)
			FSM_TEST_COND (!CH_FREE&&!BACKOFF_LIMIT_REACHED)
			FSM_TEST_COND (BACKOFF_LIMIT_REACHED)
			FSM_TEST_LOGIC ("cca")
			FSM_PROFILE_SECTION_OUT (state6_trans_conds)

			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 10, state10_enter_exec, ;, "CH_FREE&&!BACKOFF_LIMIT_REACHED", "", "cca", "transmit", "tr_46", "WSN_MAC_UP [cca -> transmit : CH_FREE&&!BACKOFF_LIMIT_REACHED / ]")
				FSM_CASE_TRANSIT (1, 11, state11_enter_exec, csma_schedule_backoff();, "!CH_FREE&&!BACKOFF_LIMIT_REACHED", "csma_schedule_backoff()", "cca", "back off", "tr_49", "WSN_MAC_UP [cca -> back off : !CH_FREE&&!BACKOFF_LIMIT_REACHED / csma_schedule_backoff()]")
				FSM_CASE_TRANSIT (2, 3, state3_enter_exec, csma_init ();, "BACKOFF_LIMIT_REACHED", "csma_init ()", "cca", "online", "tr_93", "WSN_MAC_UP [cca -> online : BACKOFF_LIMIT_REACHED / csma_init ()]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (7, "slot_intrpt", state7_enter_exec, "WSN_MAC_UP [slot_intrpt enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [slot_intrpt enter execs]", state7_enter_exec)
				{
				int i;
				double band;
				int frequency;
				if(C_SELF_NEW_SLOT)
					...
					if(g_mac_slot_number == 9)					
						{
						neighbor_table.active_time--;
						}
					if(neighbor_table.active_time == 5)					//达到保活危险值
						{
						create_actice_frame(neighbor_table.node_address,-1);
						neighbor_table.active_time = 4;
						}
			...
			FSM_STATE_EXIT_FORCED (7, "slot_intrpt", "WSN_MAC_UP [slot_intrpt exit execs]")


			/** state (slot_intrpt) transition processing **/
			FSM_TRANSIT_FORCE (3, state3_enter_exec, ;, "default", "", "slot_intrpt", "online", "tr_25", "WSN_MAC_UP [slot_intrpt -> online : default / ]")
				/*---------------------------------------------------------*/



			/** state (waiting) enter executives **/
			FSM_STATE_ENTER_UNFORCED (8, "waiting", state8_enter_exec, "WSN_MAC_UP [waiting enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [waiting enter execs]", state8_enter_exec)
				{
				int type;
				int fatherBand;
				int frequency;
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (17,"WSN_MAC_UP")


			/** state (waiting) exit executives **/
			FSM_STATE_EXIT_UNFORCED (8, "waiting", "WSN_MAC_UP [waiting exit execs]")


			/** state (waiting) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [waiting trans conditions]", state8_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "C_SELF_START_SCAN", "", "waiting", "scan", "tr_4", "WSN_MAC_UP [waiting -> scan : C_SELF_START_SCAN / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "C_SELF_START_ASSOCIATE", "", "waiting", "associating", "tr_17", "WSN_MAC_UP [waiting -> associating : C_SELF_START_ASSOCIATE / ]")
				FSM_CASE_TRANSIT (2, 8, state8_enter_exec, ;, "default", "", "waiting", "waiting", "tr_13", "WSN_MAC_UP [waiting -> waiting : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (9, "MLME", state9_enter_exec, "WSN_MAC_UP [MLME enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [MLME enter execs]", state9_enter_exec)
				{
				/* recv app data, save in subqueue */
				//int dest;
				int type;
			...
			FSM_STATE_EXIT_FORCED (9, "MLME", "WSN_MAC_UP [MLME exit execs]")


			/** state (MLME) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [MLME trans conditions]", state9_trans_conds)
			FSM_INIT_COND (g_mac_sending_flag == 2)
			FSM_TEST_COND (g_mac_sending_flag != 2)
			FSM_TEST_LOGIC ("MLME")
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 3, state3_enter_exec, ;, "g_mac_sending_flag == 2", "", "MLME", "online", "tr_36", "WSN_MAC_UP [MLME -> online : g_mac_sending_flag == 2 / ]")
				FSM_CASE_TRANSIT (1, 12, state12_enter_exec, ;, "g_mac_sending_flag != 2", "", "MLME", "send", "tr_72", "WSN_MAC_UP [MLME -> send : g_mac_sending_flag != 2 / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (10, "transmit", state10_enter_exec, "WSN_MAC_UP [transmit enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [transmit enter execs]", state10_enter_exec)
				{
				int dest = -1;
				int type = -1;
				int id = -1;
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (21,"WSN_MAC_UP")


			/** state (transmit) exit executives **/
			FSM_STATE_EXIT_UNFORCED (10, "transmit", "WSN_MAC_UP [transmit exit execs]")


			/** state (transmit) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [transmit trans conditions]", state10_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 3, state3_enter_exec, ;, "C_SELF_SLOT_COMPLETED", "", "transmit", "online", "tr_85", "WSN_MAC_UP [transmit -> online : C_SELF_SLOT_COMPLETED / ]")
				FSM_CASE_TRANSIT (1, 10, state10_enter_exec, ;, "default", "", "transmit", "transmit", "tr_99", "WSN_MAC_UP [transmit -> transmit : default / ]")
				}
				/*---------------------------------------------------------*/



			/** state (back off) enter executives **/
			FSM_STATE_ENTER_UNFORCED (11, "back off", state11_enter_exec, "WSN_MAC_UP [back off enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [back off enter execs]", state11_enter_exec)
				{
				int dest = -1;
				int type = -1;
				int id = -1;
				...
				if(g_mac_sending_flag == 4)
					{
					g_mac_current_time = op_sim_time();
					pkptr_size = op_pk_total_size_get(pkptr);
					g_mac_sending_time = pkptr_size/g_mac_data_rate;
					//printf("g_mac_sending_time = %f\n",g_mac_sending_time);
					if(g_mac_current_time + g_mac_sending_time + g_mac_backoff_duration - g_mac_slot_start_time > 0.495)
						{
						op_intrpt_schedule_self(op_sim_time(), Slot_Completed_Code);	
						}
					}
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (23,"WSN_MAC_UP")


			/** state (back off) exit executives **/
			FSM_STATE_EXIT_UNFORCED (11, "back off", "WSN_MAC_UP [back off exit execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [back off exit execs]", state11_exit_exec)
				{
				g_mac_sending_flag = 5;
				}
				FSM_PROFILE_SECTION_OUT (state11_exit_exec)


			/** state (back off) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [back off trans conditions]", state11_trans_conds)
			FSM_INIT_COND (BACKOFF_TIMER_EXPIRED && CH_FREE)
			FSM_TEST_COND (BACKOFF_TIMER_EXPIRED && !CH_FREE)
			FSM_TEST_COND (C_SELF_SLOT_COMPLETED)
			FSM_DFLT_COND
			FSM_TEST_LOGIC ("back off")
			FSM_PROFILE_SECTION_OUT (state11_trans_conds)

			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 10, state10_enter_exec, ;, "BACKOFF_TIMER_EXPIRED && CH_FREE", "", "back off", "transmit", "tr_45", "WSN_MAC_UP [back off -> transmit : BACKOFF_TIMER_EXPIRED && CH_FREE / ]")
				FSM_CASE_TRANSIT (1, 6, state6_enter_exec, ;, "BACKOFF_TIMER_EXPIRED && !CH_FREE", "", "back off", "cca", "tr_90", "WSN_MAC_UP [back off -> cca : BACKOFF_TIMER_EXPIRED && !CH_FREE / ]")
				FSM_CASE_TRANSIT (2, 3, state3_enter_exec, ;, "C_SELF_SLOT_COMPLETED", "", "back off", "online", "tr_97", "WSN_MAC_UP [back off -> online : C_SELF_SLOT_COMPLETED / ]")
				FSM_CASE_TRANSIT (3, 11, state11_enter_exec, ;, "default", "", "back off", "back off", "tr_94", "WSN_MAC_UP [back off -> back off : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (12, "send", state12_enter_exec, "WSN_MAC_UP [send enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [send enter execs]", state12_enter_exec)
				{
				int pkptr_size;
				Packet* pk2;
				if((op_intrpt_type() == OPC_INTRPT_STRM) && (op_intrpt_strm() == 3))
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (25,"WSN_MAC_UP")


			/** state (send) exit executives **/
			FSM_STATE_EXIT_UNFORCED (12, "send", "WSN_MAC_UP [send exit execs]")


			/** state (send) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_UP [send trans conditions]", state12_trans_conds)
			FSM_INIT_COND (C_SELF_SLOT_COMPLETED)
			FSM_TEST_COND (C_STRM_NWK_DATA)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 3, state3_enter_exec, ;, "C_SELF_SLOT_COMPLETED", "", "send", "online", "tr_58", "WSN_MAC_UP [send -> online : C_SELF_SLOT_COMPLETED / ]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, ;, "C_STRM_NWK_DATA", "", "send", "App_Msg_Up", "tr_67", "WSN_MAC_UP [send -> App_Msg_Up : C_STRM_NWK_DATA / ]")
				FSM_CASE_TRANSIT (2, 9, state9_enter_exec, ;, "C_STRM_NWK_MLME", "", "send", "MLME", "tr_71", "WSN_MAC_UP [send -> MLME : C_STRM_NWK_MLME / ]")
				FSM_CASE_TRANSIT (3, 12, state12_enter_exec, ;, "default", "", "send", "send", "tr_59", "WSN_MAC_UP [send -> send : default / ]")
				}
				/*---------------------------------------------------------*/
...
void
_op_WSN_MAC_UP_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_WSN_MAC_UP_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_WSN_MAC_UP_terminate ())
...
/* local variable prs_ptr in _op_WSN_MAC_UP_svar function. */
#undef subnet_objid
...
VosT_Obtype
_op_WSN_MAC_UP_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_WSN_MAC_UP_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (WSN_MAC_UP)",
		sizeof (WSN_MAC_UP_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_WSN_MAC_UP_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	WSN_MAC_UP_state * ptr;
	FIN_MT (_op_WSN_MAC_UP_alloc (obtype))

	ptr = (WSN_MAC_UP_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "WSN_MAC_UP [init enter execs]";
#endif
		}
...
void
_op_WSN_MAC_UP_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	WSN_MAC_UP_state		*prs_ptr;

	FIN_MT (_op_WSN_MAC_UP_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (WSN_MAC_UP_state *)gen_ptr;
