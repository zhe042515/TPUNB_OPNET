/* Process model C form file: WSN_MAC_DOWN.pr.c */
...
#define FIN_PREAMBLE_DEC	WSN_MAC_DOWN_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((WSN_MAC_DOWN_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
//	Packet* packet;
	...
	IE = (framecontrol>>6)&1; 
		...
		if(subtype == ASSOCIATE_REQ_FRAME && g_mac_node_status == ONLINE)				//关联请求帧处理
			{
			...
			for(i = 0;i<CHILDREN_NODE_NUM ;i++)
				...
				if(neighbor_table[i].node_id == 0)
					...
					neighbor_table[i].active_time = ACTIVE_TIME;
					create_associate_response_frame(source,2);//关联成功
					...
					if( i == CHILDREN_NODE_NUM-1)
						{
						create_associate_response_frame(source,1);//关联失败
						}
					}
				}
			}
...
extern "C" {
#endif
	void WSN_MAC_DOWN (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_WSN_MAC_DOWN_init (int * init_block_ptr);
	void _op_WSN_MAC_DOWN_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_WSN_MAC_DOWN_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_WSN_MAC_DOWN_alloc (VosT_Obtype, int);
	void _op_WSN_MAC_DOWN_svar (void *, const char *, void **);
...
void
WSN_MAC_DOWN (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	...
	FIN_MT (WSN_MAC_DOWN ());
		...
		FSM_ENTER ("WSN_MAC_DOWN")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "WSN_MAC_DOWN [init enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_DOWN [init enter execs]", state0_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (0, "init", "WSN_MAC_DOWN [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_37", "WSN_MAC_DOWN [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "WSN_MAC_DOWN [idle enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_DOWN [idle enter execs]", state1_enter_exec)
				{
				/*
			...
			FSM_EXIT (3,"WSN_MAC_DOWN")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "WSN_MAC_DOWN [idle exit execs]")


			/** state (idle) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_DOWN [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 4, state4_enter_exec, set_channel_start_send ();, "C_SELF_NEW_SLOT", "set_channel_start_send ()", "idle", "slot_intrpt", "tr_24", "WSN_MAC_DOWN [idle -> slot_intrpt : C_SELF_NEW_SLOT / set_channel_start_send ()]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, ;, "C_STRM_MSG_FROM_CHILD_NODE", "", "idle", "msg from down", "tr_29", "WSN_MAC_DOWN [idle -> msg from down : C_STRM_MSG_FROM_CHILD_NODE / ]")
				FSM_CASE_TRANSIT (2, 2, state2_enter_exec, ;, "C_STRM_NWK_DATA", "", "idle", "App_Msg_Down", "tr_31", "WSN_MAC_DOWN [idle -> App_Msg_Down : C_STRM_NWK_DATA / ]")
				FSM_CASE_TRANSIT (3, 5, state5_enter_exec, ;, "C_STRM_NWK_MLME", "", "idle", "MLME", "tr_35", "WSN_MAC_DOWN [idle -> MLME : C_STRM_NWK_MLME / ]")
				FSM_CASE_TRANSIT (4, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_34", "WSN_MAC_DOWN [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "App_Msg_Down", state2_enter_exec, "WSN_MAC_DOWN [App_Msg_Down enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_DOWN [App_Msg_Down enter execs]", state2_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (2, "App_Msg_Down", "WSN_MAC_DOWN [App_Msg_Down exit execs]")


			/** state (App_Msg_Down) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_DOWN [App_Msg_Down trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "g_mac_sending_flag == 2", "", "App_Msg_Down", "idle", "tr_30", "WSN_MAC_DOWN [App_Msg_Down -> idle : g_mac_sending_flag == 2 / ]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, ;, "g_mac_sending_flag == 1", "", "App_Msg_Down", "slot_intrpt", "tr_54", "WSN_MAC_DOWN [App_Msg_Down -> slot_intrpt : g_mac_sending_flag == 1 / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (3, "msg from down", state3_enter_exec, "WSN_MAC_DOWN [msg from down enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_DOWN [msg from down enter execs]", state3_enter_exec)
				{
				int framecontrol;
				int type;
				Packet* pk;
			...
			FSM_STATE_EXIT_FORCED (3, "msg from down", "WSN_MAC_DOWN [msg from down exit execs]")


			/** state (msg from down) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "msg from down", "idle", "tr_55", "WSN_MAC_DOWN [msg from down -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (slot_intrpt) enter executives **/
			FSM_STATE_ENTER_UNFORCED (4, "slot_intrpt", state4_enter_exec, "WSN_MAC_DOWN [slot_intrpt enter execs]")
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"WSN_MAC_DOWN")


			/** state (slot_intrpt) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "slot_intrpt", "WSN_MAC_DOWN [slot_intrpt exit execs]")


			/** state (slot_intrpt) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_DOWN [slot_intrpt trans conditions]", state4_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, set_channel_start_receive ();, "C_SENDING_COMPLETE", "set_channel_start_receive ()", "slot_intrpt", "idle", "tr_25", "WSN_MAC_DOWN [slot_intrpt -> idle : C_SENDING_COMPLETE / set_channel_start_receive ()]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "C_STRM_NWK_DATA", "", "slot_intrpt", "App_Msg_Down", "tr_51", "WSN_MAC_DOWN [slot_intrpt -> App_Msg_Down : C_STRM_NWK_DATA / ]")
				FSM_CASE_TRANSIT (2, 5, state5_enter_exec, ;, "C_STRM_NWK_MLME", "", "slot_intrpt", "MLME", "tr_58", "WSN_MAC_DOWN [slot_intrpt -> MLME : C_STRM_NWK_MLME / ]")
				FSM_CASE_TRANSIT (3, 4, state4_enter_exec, ;, "default", "", "slot_intrpt", "slot_intrpt", "tr_38", "WSN_MAC_DOWN [slot_intrpt -> slot_intrpt : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (5, "MLME", state5_enter_exec, "WSN_MAC_DOWN [MLME enter execs]")
				FSM_PROFILE_SECTION_IN ("WSN_MAC_DOWN [MLME enter execs]", state5_enter_exec)
				{
				/* recv app data, save in subqueue */
				//int dest;
				int type;
			...
			FSM_STATE_EXIT_FORCED (5, "MLME", "WSN_MAC_DOWN [MLME exit execs]")


			/** state (MLME) transition processing **/
			FSM_PROFILE_SECTION_IN ("WSN_MAC_DOWN [MLME trans conditions]", state5_trans_conds)
			FSM_INIT_COND (g_mac_sending_flag == 2)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "g_mac_sending_flag == 2", "", "MLME", "idle", "tr_36", "WSN_MAC_DOWN [MLME -> idle : g_mac_sending_flag == 2 / ]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, ;, "g_mac_sending_flag != 2", "", "MLME", "slot_intrpt", "tr_61", "WSN_MAC_DOWN [MLME -> slot_intrpt : g_mac_sending_flag != 2 / ]")
				}
				/*---------------------------------------------------------*/
...
void
_op_WSN_MAC_DOWN_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_WSN_MAC_DOWN_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_WSN_MAC_DOWN_terminate ())
...
/* local variable prs_ptr in _op_WSN_MAC_DOWN_svar function. */
#undef subnet_objid
...
VosT_Obtype
_op_WSN_MAC_DOWN_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_WSN_MAC_DOWN_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (WSN_MAC_DOWN)",
		sizeof (WSN_MAC_DOWN_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_WSN_MAC_DOWN_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	WSN_MAC_DOWN_state * ptr;
	FIN_MT (_op_WSN_MAC_DOWN_alloc (obtype))

	ptr = (WSN_MAC_DOWN_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "WSN_MAC_DOWN [init enter execs]";
#endif
		}
...
void
_op_WSN_MAC_DOWN_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	WSN_MAC_DOWN_state		*prs_ptr;

	FIN_MT (_op_WSN_MAC_DOWN_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (WSN_MAC_DOWN_state *)gen_ptr;
